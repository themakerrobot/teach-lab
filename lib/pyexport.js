...
//
//   teachlab/                     실행 라이브러리 (python/teachlab 그대로)
...
// zip 에 담을 teachlab 패키지 파일 목록.
// python/teachlab/ 에 모듈을 더하면 여기에도 더해야 한다.
const TEACHLAB_FILES = [
  '__init__.py', 'audio.py', 'classifier.py', 'cli.py',
  'embedder.py', 'landmarks.py', 'model.py', 'modelfile.py', 'preprocess.py',
];
const TEACHLAB_DIR = './python/teachlab/';
...
export async function exportPython(rec) {
  ...
  // teachlab 라이브러리를 그대로 담는다 — pip 에 올라가 있지 않아도 돌아간다
  const pkg = zip.folder('teachlab');
  const srcs = await Promise.all(TEACHLAB_FILES.map(async name => {
    const r = await fetch(TEACHLAB_DIR + name);
    if (!r.ok) throw new Error('teachlab source fetch failed: ' + name + ' ' + r.status);
    return [name, await r.text()];
  }));
  srcs.forEach(([name, text]) => pkg.file(name, text));
