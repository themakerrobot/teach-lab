...
// ═══════════════════════════════════════════════════════════
// 받은 zip 을 풀고 `pip install teachlab` 한 뒤 `python predict.py 사진.jpg`
// (소리 모델이면 `python predict.py 소리.wav`) 로 바로 돌아간다.
// 특징 뽑는 모델(.tflite/.task)까지 같이 담으므로 인터넷 없이도 동작한다.
//
...
function readme(rec, source) {
  ...
  const extras = isSound ? 'sounddevice' : 'opencv-python';
  // 이미지만 MediaPipe 없이 돈다 (LiteRT 로 .tflite 를 직접 돌린다)
  const pkg = isImage ? 'teachlab' : (isSound ? '"teachlab[sound]"' : '"teachlab[landmark]"');
...
export async function exportPython(rec) {
  ...
  if (source === 'sound') {
    ...
    zip.file('requirements.txt', 'teachlab[sound]>=0.3.0\nsounddevice>=0.4\n');
  } else {
    zip.file('predict.py', PY_PREDICT_IMAGE);
    zip.file('webcam.py', PY_WEBCAM);
    // 이미지 소스는 MediaPipe 가 필요 없다 (LiteRT 로 직접 돈다)
    zip.file('requirements.txt',
      (source === 'image' ? 'teachlab>=0.3.0\n' : 'teachlab[landmark]>=0.3.0\n') +
      'opencv-python>=4.8\n');
  }
